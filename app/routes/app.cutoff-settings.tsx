...
type CutoffSetting = { id: string; name: string; enabled: boolean; priority: number; timeValue: string; timezone: string; matchMode: string; notes: string };
      ...
      <div className="bsure-shell">
        ...
        <div className="bsure-flow">
          <section className="bsure-card">
            ...
            <p>Define a time-based condition that shipping rules can reference. Cutoff times must come from admin configuration, not hardcoded logic.</p>
            <Form method="post" style={{ marginTop: "18px" }}>
              ...
              <div className="bsure-form-row">
                <Field label="Setting name">
                  <input className="bsure-input" name="name" placeholder="e.g. Same Day Cutoff" />
                </Field>
              ...
              <div className="bsure-form-row" style={{ marginTop: "14px" }}>
                <Field label="Time value">
                  <input className="bsure-input" name="timeValue" placeholder="e.g. 14:00" type="time" />
                  <span className="bsure-help">24-hour format. Must come from admin config.</span>
                </Field>
                <Field label="Timezone">
                  <input className="bsure-input" name="timezone" placeholder="e.g. Asia/Kolkata" />
                  <span className="bsure-help">IANA timezone identifier.</span>
                </Field>
              </div>
              <div className="bsure-form-row" style={{ marginTop: "14px" }}>
                <Field label="Match mode">
                  <select className="bsure-select" defaultValue="before" name="matchMode">
                  ...
                  <span className="bsure-help">Rule applies when current time matches this condition.</span>
                </Field>
              </div>
              <div className="bsure-radio" style={{ marginTop: "16px" }}>
                <input aria-label="Enable cutoff setting" defaultChecked name="enabled" type="checkbox" />
                <span>
                  <strong>Enabled</strong>
                  <br />
                  <span className="bsure-help">Only enabled cutoff settings appear in shipping rule selectors.</span>
                </span>
              </div>
              <Field label="Notes" style={{ marginTop: "14px" }}>
                <textarea className="bsure-textarea" name="notes" placeholder="Internal notes about this cutoff condition" />
              </Field>
              <div className="bsure-actions" style={{ marginTop: "18px" }}>
                <button className="bsure-button" type="submit">Create cutoff setting</button>
              </div>
            </Form>
          </section>
          ...
          <section className="bsure-card">
            ...
            {settings.length === 0 ? (
              <p className="bsure-help" style={{ marginTop: "12px" }}>No cutoff settings yet. Create one above to reference in shipping rules.</p>
            ) : (
      ...
      <div className="bsure-rule-item-top">
        <div>
          ...
          <div className="bsure-rule-meta" style={{ marginTop: "6px" }}>
            Priority {item.priority} &nbsp;·&nbsp; {item.matchMode} {item.timeValue} {item.timezone && `(${item.timezone})`}
          </div>
          {item.notes && <div className="bsure-rule-meta">{item.notes}</div>}
        </div>
        <Form className="bsure-actions" method="post">
          <input name="id" type="hidden" value={item.id} />
          <button className="bsure-button secondary" name="intent" type="submit" value="cutoff:toggle">Toggle</button>
          <button className="bsure-button danger" name="intent" type="submit" value="cutoff:delete">Delete</button>
        </Form>
      </div>
    </article>
  );
